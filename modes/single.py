...
def _select_template() -> Optional[Path]:
    ...
    if len(entries) > 0:
        ...
        print(fmt(LABEL_CONFIRM,
            "Type the number to select, 'new' to use a different file, or press Enter to cancel."))
    else:
    ...
    while True:
        ...
        if raw.isdigit() is True and len(entries) > 0:
            ...
            if 0 <= idx < len(entries):
                ...
                # get_template_path raises ValueError when the name is not in the
                # index. Catch it explicitly and show the user-friendly message.
                # Previously this was a None check against a raises contract -- fix 1.
                # REF_ID: WEB_PY_002, WEB_PY_003
                try:
                    path: Path = get_template_path(display_name)
                except ValueError:
        ...
        if len(name_raw) > 0:
            ...
            if ok is True:
                ...
                print(fmt(LABEL_ERROR,
                    "Rhema could not save the template. You can try again from Manage Templates."))
...
def run_single_mode() -> None:
    ...
    if len(gen_result.missing_fields) > 0:
        print(fmt(LABEL_INFO,
            "Some cover page fields were left blank: " + ", ".join(gen_result.missing_fields)))
    ...
    if write_result.toc_updated is True:
        ...
        print(fmt(LABEL_INFO,
            "Open the output file in Word and press Ctrl+A then F9 to update the table of contents."))
