...
# REF_IDs: RM_DO178_001, RM_ISO_001, WEB_PY_001, RM_HCI_001
# Logic: Implicit structure detection using explicitly typed variables, strict truthiness checks, and named constants.
...
# ---------------------------------------------------------------------------
# Constants & Word heading style names
# ---------------------------------------------------------------------------
...
_MIN_HEADING_FONT_SIZE_HP: int = 28
_HEADING_1_FONT_SIZE_HP: int = 40

_NUMBERED_HEADING_RE = re.compile(r'^\d+(\.(\d+))*\.?\s+\S')
_ROMAN_HEADING_RE = re.compile(r'^(I{1,3}|IV|V|VI{0,3}|IX|X)\.\s+\S', re.IGNORECASE)
...
def _infer_heading_level(text: str, font_size_hp: Optional[int]) -> str:
    ...
    """
    # Follows DO-178C traceability for deterministic text mapping (REF_ID: RM_DO178_001)
    ...
    if _ROMAN_HEADING_RE.match(text.strip()) is not None or _LETTERED_HEADING_RE.match(text.strip()) is not None:
        return "Heading 1"
...
def _detect_docx(path: Path) -> DetectionResult:
    ...
    for i, para in enumerate(paragraphs):
        ...
        is_bold: bool = any(run.bold is True for run in para.runs if len(run.text.strip()) > 0)

        signals: List[str] = []

        if (_NUMBERED_HEADING_RE.match(text) is not None or
                _ROMAN_HEADING_RE.match(text) is not None or
                _LETTERED_HEADING_RE.match(text) is not None):
        ...
        if is_bold is True and word_count <= _MAX_HEADING_WORDS and text.endswith(".") is False:
            signals.append("bold_short")

        # Unnumbered section heading check
        if (word_count <= _UNNUMBERED_SHORT_MAX_WORDS and len(text) > 0 and text[-1] not in ".,:;?!)" and
                i + 1 < len(paragraphs) and len(paragraphs[i + 1].text.split()) > _BODY_PARAGRAPH_MIN_WORDS):
...
def _detect_txt(path: Path) -> DetectionResult:
    ...
    for i, line in enumerate(lines):
        ...
        if _NUMBERED_HEADING_RE.match(stripped) is not None:
            signals.append("numbered_pattern")
        if _ROMAN_HEADING_RE.match(stripped) is not None:
            signals.append("roman_pattern")
        if _LETTERED_HEADING_RE.match(stripped) is not None:
            signals.append("lettered_pattern")
        if _is_all_caps_short(stripped) is True:
            signals.append("all_caps_short")

        if word_count <= _MAX_HEADING_WORDS and len(stripped) > 0 and stripped[-1] not in ".,:;?!)":
